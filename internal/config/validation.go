...
// ValidateConfigMap validates the schema and internal logic of the static nodepools ConfigMap.
func ValidateConfigMap(cm *corev1.ConfigMap) error {
	...
	for _, res := range reservations {
		...
		for _, subblock := range res.GscSubblocks {
			...
			// 1. Validate subblock range format
			...
			// 2. Determine fully merged config for the subblock
			...
			// 3. Validate topology and nodeCount consistency
...
func validateNodepoolConfig(config *cloud.StaticNodePoolConfig) error {
	...
	// Enforce strictly v7x machine types
	if !strings.HasPrefix(strings.ToLower(config.MachineType), "tpu7x-") {
		return fmt.Errorf("unsupported machineType %q: static provisioner strictly supports GKE TPU v7x (machineType must start with 'tpu7x-')", config.MachineType)
	}
	...
	// 2. Calculate total chips from 3D topology dimensions
	totalChips := 1
	...
	// GKE TPU v7x (Trillium) hardware architecture strictly packs exactly 4 chips per VM host
